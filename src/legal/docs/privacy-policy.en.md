...
1. The Company entrusts sign-in authentication and the sending of push notifications to the overseas providers set out in paragraph 2 in order to provide the Service, and the corresponding items are therefore transferred abroad for processing. The server infrastructure and database are located in the Republic of Korea (Seoul).

2. Details of the transfer abroad are as follows, by recipient.
...
4. You may refuse the transfer abroad. However, because sign-in authentication is a precondition for using the Service, refusing the transfer made for authentication restricts membership registration and use of the Service; existing members may stop the transfer by withdrawing their membership. The transfer for push notifications can be refused individually by withdrawing notification permission in the device settings, in which case other features remain fully usable. Refusals are received at the contact in Article 11.
...
1. Data subjects may exercise the following rights concerning personal information protection against the Company at any time.
   ...
   - Withdrawal of consent (including withdrawal of membership)
...
2. Rights may be exercised as follows.
   ...
   - Device settings: the app access permissions in Article 14 can be changed or withdrawn at any time in the device settings.
